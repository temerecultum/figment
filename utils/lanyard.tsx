...
export function Lanyard() {
    ...
    if (activity?.listening_to_spotify) {
        ...
        return (
            <>
                <div className="font-sans text-white font-thin italic absolute top-0 left-0 text-left p-3">
                    ...
                    <Link href={`https://open.spotify.com/track/${activity?.spotify.track_id}`}>
                        ...
                        <a>
                            <h2 className="sm:text-xs lg:text-lg">🎶 {activity.spotify?.song} - {activity.spotify?.artist}</h2></a></Link>
                </div>
            </>
        );
    }
